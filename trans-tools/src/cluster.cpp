...
void VoxelGrid_LabelFilter(pcl::PointCloud<InputPointT> &InputCloudPoint, pcl::PointCloud<OutputPointT> &OutPointCloud, float X_Voxel, float Y_Voxel, float Z_Voxel, int minNum)
{
	...
	for (unsigned int cp = 0; cp < first_and_last_indices_vector.size(); ++cp)
	{
		...
		for (unsigned int li = first_index; li < last_index; ++li)
		{
			OutputPointT &cur_point = InputCloudPoint.points[index_vector[li].cloud_point_index];

			x_Sum += cur_point.x;
			y_Sum += cur_point.y;
			z_Sum += cur_point.z;
			sz++;
			inds[cur_point.label]++;
		}
		...
		float ratio = 0.0;
		for (int j = 0; j < 6; ++j) {
			if (float(inds[j]) / float(sz) > ratio) {
				ratio = float(inds[j]) / float(sz);
				PointCloud.label = j;
			}
		}
...
int main (int argc, char** argv)
{
	...
	n.param<double>(std::string("plane_threshold"), param.planeTh, 0.03);
	...
	cout << "  plane_threshold:  " << param.planeTh << endl;
    ...
    cloud_filtered->reserve(100000000);
    VoxelGrid_LabelFilter<pcl::PointXYZL, pcl::PointXYZL>(*cloud, *cloud_filtered, 
															param.leaf_size, param.leaf_size, param.leaf_size, 
															param.leaf_MinNum);
    cout << cloud_filtered->size() << " filtered points..." << endl;
    ...
    seg.setDistanceThreshold(param.planeTh);
    seg.setInputCloud(cloud_filtered);
    seg.segment(*inliers, *coefficients);

    pcl::PointCloud<pcl::PointXYZL>::Ptr cloud_f(new pcl::PointCloud<pcl::PointXYZL>);
    if (inliers->indices.size() == 0) {
        ...
        double min_z = min.z;
        std::cout << "ground plane size: " << cloud_plane->points.size()  << ", min_x:" << min.x << ", min_y:" << min.y << ", min_z:" << min.z << std::endl;
	...
	cout << "Point cloud withoud ground: " << cloud_filtered->points.size() << endl;
	...
	for (int i = 0; i < cloud_filtered->points.size(); ++i) {
		pcl::PointXYZL &cur_p = cloud_filtered->points[i];
		pcl::PointXYZRGB p;
		p.x = cur_p.x;
		p.y = cur_p.y;
		p.z = cur_p.z;
		p.r = col_map[cur_p.label][0];
		p.g = col_map[cur_p.label][1];
		p.b = col_map[cur_p.label][2];
		origin_cloud->push_back(p);
	}
	pcl::visualization::CloudViewer viewer1("Origin viewer");
    viewer1.showCloud(origin_cloud);
	...
	// 聚类
    DBSCANCluster<pcl::PointXYZL> ec;
    ec.setCorePointMinPts(20);
    ec.setClusterTolerance(param.searchRadius);
    ec.setMinClusterSize(param.min_size);
    ec.setMaxClusterSize(param.max_size);
    ...
    for (auto &cluster : cluster_indices) {
        ...
        for (auto &index : cluster.indices) {
            pcl::PointXYZL &point = cloud_filtered->points[index];
			//  cout << point.label << endl;
            colors[point.label]++;
            ++sz;
        }
        int seg_id = 0;
        for (int i = 0; i < 6; ++i) {
            if (colors[i] > colors[seg_id]) {
                seg_id = i;
                break;
            }
        }
        for (auto &index : cluster.indices) {
            ...
            point_color.r = col_map[seg_id][0];
            point_color.g = col_map[seg_id][1];
            point_color.b = col_map[seg_id][2];
            colored_cloud->push_back(point_color);
        }
    }

	cout << "Clustered points: " << colored_cloud->size() << endl;
	

	// 显示聚类
    // show_point_cloud(colored_cloud, "final result");
